    ...
    ( $(
        ...
        $(
            ...
            pub fn $name<MSG, const $M: usize, const $N: usize>(attrs: [$crate::Attribute<MSG>;$M], children: [$crate::Node<MSG>;$N]) -> $crate::Node<MSG>
                where MSG: Clone,
                {
                    $crate::html::html_element(stringify!($name), attrs.to_vec(), children.to_vec())
                }
            */

            pub fn $name<MSG,A,C>(attrs: A , children: C ) -> $crate::Node<MSG>
                where MSG: Clone,
                      A: AsRef<[$crate::Attribute<MSG>]>,
                      C: AsRef<[$crate::Node<MSG>]>
                {
                    $crate::html::html_element(stringify!($name), attrs.as_ref().to_vec(), children.as_ref().to_vec())
                }
