...
where
    ...
    T: PartialEq,
{
    diff_with_key(&"style", old, new, &"key")
}
...
{
    diff_recursive(style_literal, &old, &new, &mut 0, key)
}
    ...
    ATT: PartialEq + Ord + ToString + Clone,
{
    ...
    let new_element_style = new_element.aggregate_styles();
    let old_element_style = old_element.aggregate_styles();

    match (old_element_style, new_element_style) {
        (Some(old_style), Some(new_style)) => {
            println!("both has styles..");
            let old_styles =
                old_style.get_styles().expect("must be a style attribute");
            let new_styles =
                new_style.get_styles().expect("must be a style attribute");
            if old_styles != new_styles {
                add_attributes.push(new_style);
            }
        }
        (None, Some(new_style)) => {
            println!("is a new style");
            add_attributes.push(new_style);
        }
        (Some(_old_style), None) => {
            println!("removing old style..");
            remove_attributes.push(style_literal.clone())
        }
        (None, None) => (),
    }
